...
pub fn get_current_task_st_time() -> usize {
    let task = current_task().unwrap();
...
/// map current task's virtual memory to allocated physical page
pub fn current_task_map(start: usize, len: usize, port: usize) -> isize {
    let task = current_task().unwrap();
    let mut inner = task.inner_exclusive_access();
    inner.memory_set.checked_map(start, len, port)
}

/// unmap current task's virtual memory to allocated physical page
